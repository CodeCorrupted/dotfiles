...
This is a config file for the [Tiling Windows Manager][1] [i3][2].

It was made with this things on mind:

1. To use on Artix Linux with OpenRC as init.
2. With a %60 keyboard and only one layout, ES-latam in this case.
3. Only one Screen.

This means that you can't find an use to special keys, probably some bindings doesn't work if you have another layout or some things don't work as expected when trying to run this config file on another distro of Linux.
...
Some programs are optional, but if you want that all the keybindings work, you have to install them.
...
The main or **modifier** key is the _Windows_ or _Super_ key, from now on it will be referred to as $mod. This means that you need that key for almost every binding.
...
- $mod + r: resize. Here you can modify the sizes of the windows (if there are more than one in the workspace):
...
Some applications were assigned to specific workspaces, here I leave you the list of them:
...
First of all, i recommend you use either Vim or Neovim for this task. Just because the method of folding, you are free of use whatever you want, though.

Secondly, using the docs of I3, if you aren't familiar with this yet.

I split the config file in others files on the directory `conf.d`, where the names are self-explanatory. This was made for easy maintenance. You can change it as you like.

The only thing that I keep on the config file are: an `include` which adds all the files in the `conf.d` directory, the variable of $mod, which the others files use it, and the execution of polybar.
